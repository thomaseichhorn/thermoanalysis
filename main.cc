...
int main(int argc, char** argv)
{
	...
	//calc and output the temperature drop over the non-aluminium object
	double tempdrop = 0.02;
	double w1 = fit1->Eval(40);
	double w2 = fit2->Eval(40);
	
	cout<<"T_unten+Leitpaste= "<<w1+tempdrop<<endl;
	cout<<"T_oben-Leitpaste= "<<w2-tempdrop<<endl;
		
	double weissklebertempdrop = w2-w1-2*tempdrop;
	cout<<"Temperature drop: "<< weissklebertempdrop<<endl;
	double deltaTpromm = weissklebertempdrop/ 6.36;
	cout<<"Temperature drop per mm: "<< deltaTpromm<<endl;
	cout<<"Temperature drop per mm per K "<<deltaTpromm*2/(w1+w2)<<endl;
	

	double steigung = deltaTpromm *1000;
	double steigungAL = fit1->GetParameter(1)*1000;
	
	lambda = widerstand * stromstaerke* stromstaerke /(steigung*flaeche);
	lambda1 = widerstand * stromstaerke* stromstaerke /(steigungAL*flaeche);
	cout<<"Wärmeleitk Kleber:"<<lambda<< endl;
	cout<< "Aluminium Leitk: "<< lambda1<< endl;
	
	//text in the Plot
	TPaveText *pt = new TPaveText(0.3264094,0.9337097,0.6735906,0.995,"blNDC");
	pt->SetName("title");
	pt->SetTextFont(42);
	TText *text = pt->AddText("Temperature gradient");
	pt->Draw();  
	pt = new TPaveText(21,24.4,40,26,"br");
	text = pt->AddText("Delta T von 8,327 K");
	pt->Draw();

	
	//vertical line representing the margin between the alum-blocks
	TLine * line1 = new TLine(40,18.6,40,30);
	line1->SetLineColor(1);
	line1->Draw();
	
	TLine * line2 = new TLine(0,(w1+tempdrop),88,(w1+tempdrop));
	line2->SetLineColor(3);
	line2->Draw();
	TLine * line3 = new TLine(0,w2-tempdrop,88,w2-tempdrop);
	...
	TCanvas *c2 = new TCanvas ("Tempdiff", "transparent pad",600,400);
			
	TH2D * h2 = new TH2D("Tempdiff","Temperature Diff between adjacent sensors",100,0 ,maxtime,10,-1,10);
		
	int style2
	{
		c2->Range(156250,-0.1875,193750,0.6875);
		c2->SetFillColor(0);
		c2->SetBorderMode(0);
		c2->SetBorderSize(2);
		...
		cii = TColor::GetColor("#000099");
		h2->SetLineColor(cii);
		h2->GetXaxis()->SetTitle("Time in s");
		h2->GetXaxis()->CenterTitle(true);
		h2->GetXaxis()->SetLabelFont(42);
		h2->GetXaxis()->SetLabelSize(0.035);
		h2->GetXaxis()->SetTitleSize(0.035);
		h2->GetXaxis()->SetTitleFont(42);
		h2->GetYaxis()->SetTitle("Temperature in C");
		...
		DeltaT12->SetLineColor(1);
		DeltaT23->SetLineColor(2);
		//DeltaT34->SetLineColor(3);
		//DeltaT45->SetLineColor(4);
		DeltaT56->SetLineColor(5);
		DeltaT67->SetLineColor(6);
		DeltaT78->SetLineColor(7);
		//DeltaT89->SetLineColor(8);
		DeltaT35->SetLineColor(9);
		Delta35proT->SetLineColor(10);
		...
		leg1->SetFillColor(0);
		leg1->SetFillStyle(0);
		leg1->AddEntry(DeltaT12,"Delta12","lp");
		leg1->AddEntry(DeltaT23,"Delta23","lp");
		//leg1->AddEntry(DeltaT34,"Delta34","lp");
		//leg1->AddEntry(DeltaT45,"Delta45","lp");
		leg1->AddEntry(DeltaT56,"Delta56","lp");
		leg1->AddEntry(DeltaT67,"Delta67","lp");
		leg1->AddEntry(DeltaT78,"Delta78","lp");
		//leg1->AddEntry(DeltaT89,"Delta89","lp");
		leg1->AddEntry(DeltaT35,"Delta35","lp");
		leg1->Draw();
	}

	// draw the canvas with histogram and graph on it
	c2->Update();
	c2->Write();
	c2->Close();
	...
	TCanvas * c3 = new TCanvas("Overall Temperature", "transparent pad", 600,400);
	c3->Range(156250,13.5625,193750,22.9375);
	c3->SetFillColor(0);
	c3->SetBorderMode(0);
	c3->SetBorderSize(2);
	...
	TH2D * h3 = new TH2D("h3","Overall sensor temperatures", 10 , 0 , maxtime , 10 , 0 , 50);
	h3->SetDirectory(0);
	h3->SetStats(0);
	...
	h3->SetLineColor(ci);
	h3->GetXaxis()->SetTitle("time in s");
	...
	h3->GetYaxis()->SetTitle("Temperature in C");
	...
	Temp3->SetLineColor(3);
	Temp4->SetLineColor(4);
	Temp5->SetLineColor(5);
	Temp6->SetLineColor(6);
	Temp7->SetLineColor(7);
	Temp8->SetLineColor(8);
	Temp9->SetLineColor(9);
	...
	leg2->SetFillColor(0);
	leg2->SetFillStyle(0);
	leg2->AddEntry(Temp1,"Temperature 1","lp");
	leg2->AddEntry(Temp2,"Temperature 2","lp");
	leg2->AddEntry(Temp3,"Temperature 3","lp");
	//leg2->AddEntry(Temp4,"Temperature 4","lp");
	leg2->AddEntry(Temp5,"Temperature 5","lp");
	leg2->AddEntry(Temp6,"Temperature 6","lp");
	leg2->AddEntry(Temp7,"Temperature 7","lp");
	leg2->AddEntry(Temp8,"Temperature 8","lp");
	//leg2->AddEntry(Temp9,"Temperature 9","lp");
	leg2->Draw();
	...
	c3->Write();
	// close the canvas, so nothing else is written on it
	c3->Close();
